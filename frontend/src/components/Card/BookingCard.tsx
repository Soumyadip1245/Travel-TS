...
import Itinery from '../Itinery/Itinery';
...
const BookingCard: React.FC<BookingCardProps> = ({
    ...
    const [itinery, setItinery] = useState<boolean>(false)
    ...
    if (itinery) return <Itinery booking={booking} isUser={isUser!}/>
    return (
        <div className="flex flex-col w-full p-6 bg-gray-800 text-white rounded-lg shadow-md mb-4 border border-gray-700">
            ...
            {!isUser && (
                <table className="table-auto w-full text-left border-separate border border-gray-600 mb-4">
                    <thead>
                        <tr className="bg-gray-700 text-gray-300">
                            <th className="p-2 border-b border-gray-600">Total Price</th>
                            <th className="p-2 border-b border-gray-600">Supplier Payment Amount</th>
                            <th className="p-2 border-b border-gray-600">GST (18%)</th>
                            <th className="p-2 border-b border-gray-600">Commission ({booking.package?.percentageIncome}%)</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        <tr>
                            ...
                            <td className="px-4 py-2">
                                ₹{(booking.supplierPaymentAmount -
                                    (booking.supplierPaymentAmount * (booking.package?.percentageIncome! / 100))
                                ).toFixed(2)}
                            </td>                            <td className="px-4 py-2">₹{(booking.totalPrice - booking.supplierPaymentAmount).toFixed(2)}</td>
                            <td className="px-4 py-2">₹{booking.supplierPaymentAmount * (booking.package?.percentageIncome! / 100)}</td>
                        </tr>
                    </tbody>
                </table>
            )}
